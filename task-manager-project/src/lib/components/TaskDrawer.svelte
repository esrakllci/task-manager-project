<script>
	...
	const handleSubmit = () => {
		let newTask = {
			id: Date.now(), //Kullanıldığındaki anı alır.
			title: title,
		...
		AllTasks.update((currentTasks) => {
			return [newTask, ...currentTasks]; //Sadece yeni taskler eklenmesin mevcut taskler de eklensin
			// diye spread yani üç nokta operatörü kullanıldı.
		});
		title = ''; //kullanıcı tek tek silmek zorunda kalmasın diye bu alanların temizlenmesi işlemi
		description = '';
		personel = [];
		deadline = '';
	};
